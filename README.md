...
1. 添加模型，填写名称、协议、API 地址、模型 ID 和 API Key。可以先保存无密钥模型。
2. 按服务商真实能力填写高级设置。新模型默认勾选工具调用、图像输入；已保存的关闭选项保持不变。Codex 必须填写上下文窗口；推理档位默认留空。
3. 点击“应用到 Agent”，选择兼容的软件，核对目标文件和脱敏前后内容。
4. 点击“确认覆盖并备份”后才写入。若预览期间文件发生变化，需重新预览。
5. WorkBuddy 在新会话手动选择模型；Codex 桌面端完全退出后重新打开并新建会话。Claude Code 新建会话读取配置。
...
当前版本的“技能”入口暂时禁用，显示“待开放”，以下能力保留在代码中，待完善后开放。侧边栏顺序为“模型库”“模型配置记录”“技能”“设置”。
